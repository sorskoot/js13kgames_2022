...
const STATES = { START: 1, GAME: 2, LOSE: 3, INIT: 0 };
...
class App {
    ...
    createTitles() {
        this.titleParent = new BABYLON.Node("titleParent");

        const pressTrigger = BABYLON.MeshBuilder.CreatePlane("PressTrigger", { width: 3, height: .75 });
        pressTrigger.parent = this.titleParent;
        pressTrigger.position = new BABYLON.Vector3(0, 1, 2);
        
        var pressTriggerTexture = new BABYLON.DynamicTexture("dynamic texture", { width: 1200, height: 300 });
        var textureContext = pressTriggerTexture.getContext();
        var pressTriggerMaterial = new BABYLON.StandardMaterial("Mat");
        ...
        pressTrigger.material = pressTriggerMaterial;
         ...
         pressTriggerTexture.drawText("Press trigger to start", 300, 215, font, "#7c8898", "rgba(0,0,0,0)", true, true);
        ...
        image.onload = () => {
            ...
            ctx.strokeStyle = '#222';
            ctx.lineWidth = 12;
            ctx.strokeText("DEATHKEEPER", 20, 128);

            ctx.fillStyle = "#FFF";
            ctx.fillText("DEATHKEEPER", 25, 123);
            ctx.shadowColor = '#888';
            ctx.shadowBlur = 6;
            ctx.shadowOffsetX = -2;
            ctx.shadowOffsetY = 3;
            ctx.lineWidth = 4;
            ctx.strokeStyle = '#FFF';
            ctx.strokeText("DEATHKEEPER", 25, 123);
            
            for (var j = 0; j < 300; j += 8) {
                for (var i = 0; i < 1200; i += 8) {
                    var d = ctx.getImageData(i, j, 1, 1);
                    var r = Math.floor(d.data[0] / 16);
                    if (r != 0) {
                        var s = spritectx.getImageData(Math.floor(Math.random() * 16)+48, 17 - r, 1, 1);
